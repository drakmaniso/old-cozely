...
that they are specified and stored in column-major order, as this is what 
GLSL expects. So when writing literals, remember to use the transpose of the
mathematical notation. In other words: 
    m := Mat3{
		{a, b, c},
		{d, e, f},
		{g, h, i},
...
... corresponds to the following mathematical notation:
	...
	⎢ b  e  h⎥
	⎣ c  f  i⎦
The same inversion happens with indices: 
	m[column][row] 
... corresponds to the mathematical indices (row,column).
...
In order to avoid that, you can write a small local function:
