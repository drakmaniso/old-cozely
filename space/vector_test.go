...
import (
	...
	"github.com/drakmaniso/carol/core/space"
)
...
func TestHomogen_Dehomogenized(t *testing.T) {
	a := space.Homogen{X: 1.1, Y: 2.2, Z: 3.3, W: 4.4}
	b := a.Coord()
...
func TestCoord_Plus(t *testing.T) {
	a := space.Coord{X: 1.1, Y: 2.2, Z: 3.3}
	b := space.Coord{X: 4.4, Y: 5.5, Z: 6.6}
	c := a.Plus(b)
