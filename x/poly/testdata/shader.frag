...
vec3 glam_BaseColor;
float glam_Smoothness = 0.4;
float glam_Metallic  = 0.0;
float glam_Reflectance = 0.5;

float glam_Subsurface = 0.25;

float glam_Roughness;
vec3 glam_F0;

vec3 glam_SubsurfaceColor = vec3(0.6, 0.16, 0.48);
// vec3 glam_SubsurfaceColor = vec3(1.0, 0.0, 0.7);
...
vec3 pbr_lightingBurleySubsurface(vec3 L, vec3 V, vec3 N)
{
  ...
  return NdotL * (specular + mix(diffuse, ss*glam_SubsurfaceColor, glam_Subsurface));
  // (division by PI omitted, it's factored in light intensity)
}
...
void main(void) {
  ...
  // vec3 L = normalize(vec3(0.0, 1.0, 0.0)); //w_direction_to_sun;
  ...
  // vec3 luminance = glam_SunIlluminance * pbr_lightingNormalizedBurley(V, V, N) + ambient_luminance * pbr_lightingNormalizedBurley(-V, V, N) + glam_BaseColor * ambient_luminance;
  // vec3 luminance = 1.0*glam_SunIlluminance * pbr_lightingNormalizedBurley(V, V, N) + 1.0*glam_BaseColor * ambient_luminance;
  // vec3 luminance = 0.5*glam_SunIlluminance * pbr_lightingBurley(V, V, N) + 2.0*glam_BaseColor * (vec3(160.0/256.0, 160.0/256.0, 160.0/256.0) * 29000.0 / PI);
  // vec3 luminance = 1.0*glam_SunIlluminance * pbr_lightingBurley(V, V, N) + 1.0*glam_BaseColor * (vec3(170.0/256.0, 110.0/256.0, 190.0/256.0) * 29000.0 / PI);
  // vec3 luminance = 1.0*glam_SunIlluminance * pbr_lightingBurley(normalize((2*V + vec3(0,1,0))), V, N) + 1.0*glam_BaseColor * (vec3(170.0/256.0, 110.0/256.0, 190.0/256.0) * 29000.0 / PI);
  // vec3 luminance = 1.0*glam_SunIlluminance * pbr_lightingBurley(vec3(0,1,0), V, N) + 1.0*glam_BaseColor * (vec3(170.0/256.0, 110.0/256.0, 190.0/256.0) * 29000.0 / PI);

  // vec3 luminance = glam_SunIlluminance * pbr_lightingNormalizedBurley(V, V, N)
	// 	+ ambient_luminance * pbr_lightingNormalizedBurley(vec3(0, 1, 0), V, N)
	// 	+ ambient_luminance * pbr_lightingNormalizedBurley(vec3(0, -1, 0), V, N)
	// 	+ ambient_luminance * pbr_lightingNormalizedBurley(vec3(1, 0, 0), V, N)
	// 	+ ambient_luminance * pbr_lightingNormalizedBurley(vec3(-1, 0, 0), V, N)
	// 	+ ambient_luminance * pbr_lightingNormalizedBurley(vec3(0, 0, 1), V, N)
	// 	+ ambient_luminance * pbr_lightingNormalizedBurley(vec3(0, 0, -1), V, N);
